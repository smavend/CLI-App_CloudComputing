...
function isValidCIDR(cidr) {
	// Expresión regular para validar el formato CIDR
	const cidrRegex = /^(\d{1,3}\.){3}\d{1,3}\/\d{1,2}$/
	if (!cidrRegex.test(cidr)) {
		return "El formato del CIDR no es válido."
	}

	const [ip, prefix] = cidr.split("/")
	const prefixInt = parseInt(prefix, 10)

	// Validar que el prefijo esté en el rango adecuado
	if (prefixInt < 0 || prefixInt > 32) {
		return "El prefijo del CIDR debe estar entre 0 y 32."
	}

	// Validar que cada octeto de la IP esté en el rango de 0 a 255
	const octets = ip.split(".")
	for (let octet of octets) {
		const octetInt = parseInt(octet, 10)
		if (octetInt < 0 || octetInt > 255) {
			return "Cada octeto de la dirección IP debe estar entre 0 y 255."
		}
	}
...
<<<<<<< HEAD
	...
	#options_create_vm = [
		...
		},
		{
			...
			name: "vm_image",
			message: "Seleccione el sistema operativo de la máquina virtual: ",
			choices: [{ name: "Cirros", value: "26a1ed6a-eed7-429b-9f0c-b0480a27af2d" }],
		},
		{
			type: "list",
			name: "flavor",
			...
			choices: [
				{
					name: "RAM: 512MB CPUs: 1 Almacenamiento: 4GB",
					value: "343e7c42-ac61-463e-8aef-64aee5e0a07a",
				},
				{
					name: "RAM: 512MB CPUs: 1 Almacenamiento: 2GB",
					value: "79905b60-28e3-4773-99db-e0db124eea4c",
				},
				{
					name: "RAM: 512MB CPUs: 1 Almacenamiento: 1GB",
					value: "0dfd178b-afc3-4b7d-83f6-fa7bf7703561",
				},
			],
		},
		{
	...
	#options_pre_deploy_slice = [
		{
			...
			name: "option",
			message: "Seleccione una opción:",
			choices: [{ name: "Asignar IP del slice (CIDR)" }, { name: "Regresar" }],
		},
	]
	...
	#enter_IP = [
		{
			type: "input",
			name: "cidr",
			message: "Ingrese el CIDR para el despliegue: ",
	...
	async create_slice_from_scratch(SLICE, structure_created) {
		...
		while (true) {
			...
			if (answer.option === "Regresar") {
				...
				const cidr = await inquirer.prompt(this.#enter_IP)
				SLICE.deployment.details.cidr = cidr.cidr
				SLICE.deployment.details.ip_version = 4
	...
	async deploy_slice(SLICE) {
		...
		const result = await response.json()
		console.log(
			`El slice ${SLICE.deployment.details.project_name} está en cola para el despliegue`
		)
	}

	async create_vms_and_links(SLICE, structure_created) {
		let STRUCTURE = structure_created
			? structure_created
			: { visjs: { nodes: {}, edges: {} }, metadata: { edge_node_mapping: {}, nodes: {} } }
	...
	async delete_vm(STRUCTURE) {
		...
		STRUCTURE.metadata.edge_node_mapping = Object.fromEntries(
			Object.entries(STRUCTURE.metadata.edge_node_mapping).filter(
				([key, value]) => key != answer.vm_name
			)
		)
	}
	...
	async delete_link(STRUCTURE) {
		...
		STRUCTURE.metadata.edge_node_mapping = Object.fromEntries(
			Object.entries(STRUCTURE.metadata.edge_node_mapping).filter(
				([key, value]) => key != answer.link_name
			)
		)
	}
	...
	async set_draft_slices() {
		...
		for (let slice of result.slices) {
			this.#options_create_slice[0].choices.unshift({
				name: "Continuar con slice: " + slice.deployment.details.project_name,
				value: slice._id,
			})
		}
...
>>>>>>> 756bbcc876f4da1a1e6cb78783e8708423ffe2b0
}
